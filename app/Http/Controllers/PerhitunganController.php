...
class PerhitunganController extends Controller
{
    public function index()
    {
        ...
        if (!$checkHasEmptyData) {
            $nilaiBobotGroupByAlternatifId = DB::table('nilai_bobot')
                ...
                ->select('nilai_bobot.alternatif_id', 'alternatif.code', 'alternatif.nis', 'alternatif.kelas', 'alternatif.nama')
    ...
    protected function check_nilai_bobot_has_empty_data()
    {
        ...
        $nilaiBobotGroupByAlternatifId = DB::table('nilai_bobot')
            ->join('alternatif', 'nilai_bobot.alternatif_id', '=', 'alternatif.id')
            ->select('alternatif_id', 'code', 'nis', 'kelas', 'nama')
    ...
    protected function calculate()
    {
        $nilaiBobotGroupByAlternatifId = DB::table('nilai_bobot')
            ->join('alternatif', 'nilai_bobot.alternatif_id', '=', 'alternatif.id')
            ->select('nilai_bobot.alternatif_id', 'alternatif.code', 'alternatif.nis', 'alternatif.nama')
            ->orderBy('nilai_bobot.alternatif_id')
            ->groupBy('nilai_bobot.alternatif_id')
            ->get();
        $persentaseBobot = $this->persentase_bobot();
        ...
        for ($i = 0; $i < count($nilaiBobotGroupByAlternatifId); $i++) {
            ...
            $result[] = ['alternatif_code' => $nilaiBobotGroupByAlternatifId[$i]->code, 'nis' => $nilaiBobotGroupByAlternatifId[$i]->nis, 'nama' => $nilaiBobotGroupByAlternatifId[$i]->nama, 'hasil_perh' => $HasilPerhitungan];
        }
    ...
    protected function ranking()
    {
        ...
        for ($i = 0; $i < count($nilaiBobotGroupByAlternatifId); $i++) {
            ...
            $result[] = ['alternatif_code' => $nilaiBobotGroupByAlternatifId[$i]->code, 'nis' => $nilaiBobotGroupByAlternatifId[$i]->nis, 'kelas' => $nilaiBobotGroupByAlternatifId[$i]->kelas, 'nama' => $nilaiBobotGroupByAlternatifId[$i]->nama, 'hasil_rank' => $rank];
        }
